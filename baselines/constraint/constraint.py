...
def compute_reacher_actuation_constraint(threshold, limit=3):
    ...
    for i in range(limit + 1):
        ...
        candidates = map(lambda x: ''.join([str(hex(a)[2:]) for a in x]), candidates)
        candidates = '|'.join(list(map(lambda s: '({})'.format(s), candidates)))
        candidates_list.append(candidates)
    return '|'.join(candidates_list)


REACHER_ACTUATION = compute_reacher_actuation_constraint(4)
print(len(REACHER_ACTUATION))
